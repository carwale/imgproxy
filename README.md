...
Prereqs: Go toolchain and `libvips` per upstream requirements.
...
For container builds, see `Dockerfile` and `docker/imgproxy-build-package`.

## Upstream docs

General `imgproxy` behavior, all image operations, and advanced tuning are covered in the upstream docs at `https://docs.imgproxy.net`. This fork preserves upstream processing semantics and adds the features outlined above.

## License

This project retains the upstream `imgproxy` license. See `LICENSE` and `NOTICE`.
